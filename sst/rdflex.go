...
const (
	...
	tokenLiteralDecimal    // RDF literal (arbitrary-precision decimal)
	...
	tokenCollectionEnd     // ')'
	tokenGraphStart        // '{' for TriG
	tokenGraphEnd          // '}' for TriG
)
...
// the template lexer in Go's standard library, and is governed by a BSD license
// and Copyright 2011 The Go Authors.
type lexer struct {
	rdr *bufio.Reader

	input    []byte     // the input being scanned (should not include newlines)
	...
	for i := 0; i < len(r); {
		...
		case '\\':
			...
			case 'u':
				...
				// we can safely assume no error, because we already verified
				// the escape sequence in the lex state functions
...
// emit publishes a token back to the consumer.
...
func lexAny(l *lexer) stateFn {
	...
	case ')':
		...
		return lexAny
	case '{':
		l.emit(tokenGraphStart)
		return lexAny
	case '}':
		l.emit(tokenGraphEnd)
		return lexAny
	case '.':
		if isDigit(l.peek()) {
...
func hasValidScheme(l *lexer) bool {
	...
	// read first rune, must be in set [a-zA-Z]
	r, w := readRune(l.input[l.start:])
	if !isAlpha(r) {
		return false
	}
...
func _lexIRI(l *lexer) (stateFn, bool) {
	...
	for {
		r := l.next()
		if r == eof {
			return l.errorf("bad RdfIRI: no closing '>'"), false
		}
		for _, bad := range badIRIRunes {
			if r == bad {
				return l.errorf("bad RdfIRI: disallowed character %q", r), false
			}
		}
		...
		if r == '\\' {
			...
			case 'u':
				...
				if !l.acceptRunMin(hex, 4) {
					return l.errorf("bad RdfIRI: insufficient hex digits in unicode escape"), false
				}
				...
				for _, bad := range badIRIRunesEsc {
					if rune(i) == bad {
						return l.errorf("bad RdfIRI: disallowed character in unicode escape: %q", string(l.input[l.pos-6:l.pos])), false
					}
				}
				l.unEsc = true
			case 'U':
				...
				for _, bad := range badIRIRunesEsc {
					if rune(i) == bad {
						return l.errorf("bad RdfIRI: disallowed character in unicode escape: %q", string(l.input[l.pos-9:l.pos])), false
					}
				}
			...
			case eof:
				return l.errorf("bad RdfIRI: no closing '>'"), false
			default:
				return l.errorf("bad RdfIRI: disallowed escape character %q", esc), false
			}
		}
		if maybeAbsolute && r == ':' {
...
outer:
	...
	case '^':
		if l.next() != '^' {
			return l.errorf("bad literal: invalid datatype RdfIRI")
		}
		...
		for {
			...
			case r == '.':
				...
				if !isDigit(p) && p != 'E' && p != 'e' {
					...
					l.pos-- // backup() may already be called
					break outer
				}
				gotDot = true
			case r == 'e', r == 'E':
...
outerLoop:
	for r = l.next(); isPnLocalMid(r); r = l.next() {
		if r == '\\' {
			p := l.next()
			for _, esc := range pnSstLocalEsc {
				if esc == p {
					l.unEsc = true
					continue outerLoop
				}
			}
			return l.errorf("invalid escape character %q", p)
		}
